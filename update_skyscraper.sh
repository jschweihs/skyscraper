...
LATEST=`wget -q -O - "https://api.github.com/repos/jschweihs/skyscraper/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/'`
...
then
    ...
    wget -N https://github.com/jschweihs/skyscraper/archive/${LATEST}.tar.gz || handle_error "fetch"
    echo "--- Unpacking ---"
